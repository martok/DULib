...
interface
  ...
  const
    ...
    RTCtop = 127 ;
  type
    RTCaddr = RTCbot..RTCtop ;
    RTCrec = record rCy, rYr, rMo, rDy,
                    rHr, rMi, rSc : byte;
             end { 2 nibbles } ;
  const
    CCinCMOS : RTCaddr = $32 { Bank 1, $48 ; $7F } ;


procedure GetRTCDirectly(var RTC : RTCrec) ;
function SetSystemTimeFromRTC(RTC:RTCRec):boolean;
procedure ResetSysTime;
function DateTimeStringRTC(const RTC : RTCrec) : String;

implementation

const
  DigitString : string [16] = '0123456789ABCDEF' ;
var DigitArray : array [-1..15] of char absolute DigitString ;

function NN(const n2 : byte) : String { n2 is BCD nibbles } ;
begin
  NN:= DigitArray[n2 shr 4] + DigitArray[n2 and $F]
end ;

function DateTimeStringRTC(const RTC : RTCrec) : String;
begin
  with RTC do DateTimeStringRTC :=
    ' RTC: '+NN(rCy)+NN(rYr)+'-'+NN(rMo)+'-'+NN(rDy) +
    #32+NN(rHr)+':'+NN(rMi)+':'+NN(rSc)
end;

procedure WaitForUIPlowCLI ; assembler { Vlado+JRS };
asm
    CLI;
@@wait0:
    STI;
    CLI;            {allow interrupts in during wait }
    mov al,0ah;
    out 70h,al;
    in al,71h       { fix the UIP flag in bit 7 of al }
    or al,al;
    js @@wait0      { loop the test if it is set }
end {WaitForUIPlowCLI} ;
...
function UTCToTZ(time:TSystemTime): TSystemTime;
var dt:TDateTime;
begin
  dt:= SystemTimeToDateTime(Time);
  dt:= dt + TimeZoneBias;
...
begin
  with Rtc do begin
    TimeStruct.wYear        := StrToInt(NN(rCy)+NN(rYr));
    TimeStruct.wMonth       := StrToInt(NN(rMo));
    TimeStruct.wDay         := StrToInt(NN(rDy));
    TimeStruct.wHour        := StrToInt(NN(rHr));
    TimeStruct.wMinute      := StrToInt(NN(rMi));
    TimeStruct.wSecond      := StrToInt(NN(rSc));
  end;
  TimeStruct:= UTCToTZ(TimeStruct);
